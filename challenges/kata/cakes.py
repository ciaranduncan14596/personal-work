...
def cakes(whats_needed, what_we_have):
    items_we_need = []
    items_we_have = []
    for key, value in whats_needed.items():
        items_we_need.append(key)
    for key, value in what_we_have.items():
        items_we_have.append(key)

    for item in items_we_need:
        if item not in items_we_have:
            return 0

    numbers = []
    for item in items_we_need:
        numbers.append(int(what_we_have[item]/ whats_needed[item]))
    numbers.sort()
    return numbers[0]
