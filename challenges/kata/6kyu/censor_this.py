...
def censor_this(text, forbidden_words):
    ...
    for text in text_array:
        if text.lower() in forbidden_words:
            new_array.append('*' * len(text))
        else:
            new_array.append(text)
    return ' '.join(new_array)


b = censor_this("The cat does not like the therapy",["the","like"])
# b = censor_this("Javascript is the BEST programming language and LOLCODE is the Worst",["worst","best"])
print(b)
